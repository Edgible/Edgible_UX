...
Each series index opens with a topology diagram. They are generated by
`scripts/gen_diagrams.py` from a short spec of callers, published hostnames with
their auth modes, loopback ports and notes, and the build regenerates them, so
editing an SVG under `static/images/diagrams/` by hand achieves nothing. Change
the spec.

Two files per series, light and dark, referenced with `#only-light` and
...
same rules. Ports, hostnames and auth modes must match the chapters.
