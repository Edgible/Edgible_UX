...
Everything later in this series — the phone opening the canvas, GitHub firing a webhook — assumes there is something on this box worth publishing. Right now there is nothing, so this chapter installs it: n8n as one process on loopback **5678**, and not one byte of it on the internet.

Starting on loopback is a deliberate choice, not a stepping stone you rush past. The tempting shortcut is to bind n8n to `0.0.0.0` and forward the port on the router, which hands a credential store holding every API key you own to anyone who scans that address. Edgible does not need that: the serving agent on this guest dials **out** on 443 and reaches n8n over loopback, so the process can stay private and still be published later. Later you will give this one process **two** public hostnames with **two** locks — **org** for the editor you log into, **None** for the webhooks GitHub and `curl` must reach without a login.
