...
This is the demo of **internet publishing** and **per-app auth**. n8n is **one** process on loopback **5678**. Edgible publishes it as **two** apps: **n8n** (**org**) is the editor; **n8n-hooks** (**None**) is the inbound door. Auth is a property of the hostname, not of a path. Without that split you either lock the webhooks (GitHub never gets in) or you unlock the canvas (anyone who finds the host gets the editor). Chapters 1–5 are the demo; [6](06-n8n-teardown.md) is teardown. Control UI is [2. OpenClaw on Edgible](../openclaw-on-edgible/README.md). Pointing n8n (and OpenClaw) at a **big** local or remote LLM is [3. LLM on Edgible](../llm-on-edgible/README.md) when that model is up.
