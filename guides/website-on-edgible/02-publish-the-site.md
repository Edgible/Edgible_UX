...
The hostname you get is `site.<org>.edgible.com` — **zero-DNS publish** on first create: certificate and routing in place with no DNS step (see [Glossary](../../glossary.md)). It is worth knowing from the outset that this is not the final word on the address. A site hosted on the machine under your desk can answer at `www.example.com` instead, with a certificate issued for that name, so a visitor has no way to tell where it is served from. What that costs you is one DNS record: a `CNAME` from your name to the Edgible hostname, which means you need a domain whose DNS you can edit. That is the optional 2.6 at the end of this chapter.
