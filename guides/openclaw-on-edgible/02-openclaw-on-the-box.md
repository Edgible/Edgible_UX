...
1. `flash` in the name, no `pro`, no `preview`: `google/gemini-2.5-flash`, `google/gemini-3-flash`, `google/gemini-flash-latest`.
2. If several Flash rows: prefer the latest or highest 2.x or 3.x Flash, not `-lite`, not `-thinking`.
...
You want `bind` = loopback (or equivalent), and `ss` showing `127.0.0.1:18789` (and maybe `[::1]:18789`). `0.0.0.0:18789` or `*:18789` means it is listening on every interface, which this guide does not want. NAT still hides that from the internet, but do not leave it that way.
...
That launches the VM browser onto the Control UI (a short-lived handoff; you will not normally have to paste `gateway.auth.token`). Send `hello`.

`curl` → `200` only means HTML is served. `openclaw dashboard` is the check that the browser and WebSocket path works.
